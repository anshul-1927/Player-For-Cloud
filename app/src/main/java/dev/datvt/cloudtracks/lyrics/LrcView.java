...
public class LrcView extends View implements ILrcView{
	...
	public LrcView(Context context,AttributeSet attr){
		...
		Typeface typeface = Typeface.createFromAsset(context.getAssets(), "fonts/bryantlg.ttf");
		Typeface bold = Typeface.create(typeface, Typeface.BOLD);
		mPaint.setTypeface(bold);
//		setLoadingTipText(context.getString(R.string.noti_update_lyrics));
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (highlightText.equals("")) {
			highlightText = "...";
		}
		int highlightRowY = 50;
		mPaint.setColor(mHignlightRowColor);
		mPaint.setTextSize(mLrcFontSize);
		mPaint.setTextAlign(Align.CENTER);
